...
const PricingPlanComponent = ({plan_name, plan_desc, plan_price,bg_purple, plan_benefits, plan_btn}) => {
  return (
    <div className='max-h-[500px] w-full md:max-w-[300px] border-2 rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300'>
      ...
      <div className={`${bg_purple===true ? "bg-[#999999] h-full" : "bg-white "} text-black px-4 py-8`}>
        <p className="text-center capitalize text-sm font-semibold tracking-wide text-black">{plan_desc}</p>
        <h2 className="text-6xl md:text-5xl lg:text-6xl text-black font-black text-center my-5">{plan_price}</h2>
        <h3 className="text-lg text-center font-semibold my-5">What Comes With It?</h3>
        <ul className="px-5">
          {
            plan_benefits.map((item, index)=>(
              <li key={index} className="text-black py-1 list-item list-disc ">{item}</li>
            ))
          }
        </ul>
